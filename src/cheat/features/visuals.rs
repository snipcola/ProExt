...
use crate::{ui::main::{color_u32_to_f32, rectangle_filled}, utils::config::Config};
...
pub fn render_head_shot_line(ui: &mut Ui, window_width: i32, window_height: i32, fov: i32, view_angle_x: f32, config: Config) {
    ...
    rectangle_filled(ui, Vector2 { x: pos.x - 20.0, y: pos.y }, Vector2 { x: 17.0, y: 3.0 }, color_u32_to_f32(config.head_shot_line_color).into(), 1.0, 0);
    rectangle_filled(ui, Vector2 { x: pos.x + 6.0, y: pos.y }, Vector2 { x: 17.0, y: 3.0 }, color_u32_to_f32(config.head_shot_line_color).into(), 1.0, 0);
}